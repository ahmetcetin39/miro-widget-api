...
class MiroApplicationIntegrationTests {
    ...
    void updateWidget_whenRequestIsInvalid_thenReturnBadRequest() throws Exception {
        UpdateWidgetRequest request = UpdateWidgetRequest.builder().build(); // Mandatory fields are not set.
    ...
    void updateWidget_whenRequestIsValid_thenReturnUpdatedWidget() throws Exception {
        ...
        UpdateWidgetRequest updateRequest = UpdateWidgetRequest.builder().id(widget.getId()).xIndex(5).yIndex(0).width(5).height(5).build();
        mockMvc.perform(put(WIDGET_API_PATH)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteWidget_whenIdNotFound_thenReturnNotFound() throws Exception {
        mockMvc.perform(delete(WIDGET_API_BY_ID_PATH, 9999)
                .contentType(MediaType.APPLICATION_JSON))
    ...
    void deleteWidget_whenIdFound_thenDeleteWidget() throws Exception {
        ...
        mockMvc.perform(delete(WIDGET_API_BY_ID_PATH, widget.getId())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    }
    ...
    void findById_whenIdNotFound_thenReturnNotFound() throws Exception {
        mockMvc.perform(get(WIDGET_API_BY_ID_PATH, 9999)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound());
    }
    ...
    void findById_whenIdFound_thenReturnFoundWidget() throws Exception {
        ...
        MvcResult findByIdResponse = mockMvc.perform(get(WIDGET_API_BY_ID_PATH, createdWidget.getId())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void findAll_whenCoordinatesAreValid_thenReturnFoundWidgets() throws Exception {
        CreateWidgetRequest createRequest1 = CreateWidgetRequest.builder().xIndex(1050).yIndex(1050).width(100).height(100).build();
